...
const QuizEditor = () => {
  ...
  // Fetch quiz if in edit mode
  const { data: quizData, isLoading: isFetching } = useQuery({
    queryKey: ["quiz-edit", quizId],
    queryFn: () => axiosInstance.post("/user-quiz/get-quiz-by-id", { quizId, userId: dbUser?.id }),
  ...
  // Sync structure when quizType changes
  useEffect(() => {
    if (isEditMode) return; // Don't reset if we are editing an existing quiz (unless user changes type)
    ...
    setQuestions((prev) =>
      prev.map((q) => {
        if (quizType === "MCQ") {
          ...
          return {
            ...q,
            answer: q.answer === "True" || q.answer === "False" ? q.answer : "True",
            options: ["True", "False"],
          };
        } else {
          // Flashcard
  ...
  const addQuestion = () => {
    const newQuestion: Question =
      quizType === "MCQ"
        ...
        ? { question: "", answer: "True", options: ["True", "False"], reason: "" }
        : { question: "", answer: "" };
  ...
  const validateQuiz = () => {
    ...
    if (quizType === "MCQ" && questions.some((q) => q.options?.some((opt) => !opt.trim()))) {
              ...
              {quizType !== "Flashcard" && (
                <div>
                  ...
                  <input
                    ...
                    value={q.reason || ""}
                    onChange={(e) => handleQuestionChange(qIndex, "reason", e.target.value)}
                  />
                </div>
              )}
            </div>
