...
const SaveQuizModal = ({
  ...
  const saveQuiz = async () => {
    ...
    setIsDisabled(true);
    try {
      // Determine final quiz type for DB
      const finalQuizType = 
  ...
  return (
    <AlertModal onClose={onClose} isOpen={isOpen}>
      <div className="flex flex-col gap-y-4">
        <div>
          <h1 className="dark:text-white font-bold text-2xl mb-1">Save this quiz</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400">
            {isLimitReached 
              ? <span className="text-red-500 flex items-center gap-1 font-medium"><AlertCircle size={14} /> Limit reached: {quizCount}/{maxNumberOfQuizzes} quizzes</span>
        ...
        <div className="space-y-2">
          ...
          <div className="flex justify-between items-start">
            ...
            <p className={`text-xs ${quizTitle.length > 50 ? "text-red-500" : "text-gray-400"}`}>
              {quizTitle.length}/50
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          >
            <div className="flex items-center gap-3 mb-1">
              <Globe size={18} className={isPublic ? "text-cta" : "text-gray-400"} />
              <span className={`font-semibold ${isPublic ? "text-cta" : "dark:text-white"}`}>Public</span>
            </div>
          ...
          >
            <div className="flex items-center gap-3 mb-1">
              <Lock size={18} className={!isPublic ? "text-cta" : "text-gray-400"} />
              <span className={`font-semibold ${!isPublic ? "text-cta" : "dark:text-white"}`}>Private</span>
            </div>
        ...
        {isLimitReached && (
          <div className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 p-3 rounded-lg flex items-start gap-2">
            ...
            <p className="text-sm text-red-700 dark:text-red-400">
              You've reached your limit of {maxNumberOfQuizzes} quizzes. Delete some existing quizzes to save new ones.
            </p>
          </div>
        )}
