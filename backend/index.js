...
app.get("/api/v1/update-limit",
    ...
    async (_, res) => {
        try {
            ...
            const isNewDay = !lastUpdate || new Date(lastUpdate.setHours(0, 0, 0, 0)) < today;
            if (isNewDay) {
            ...
            const ttlInSeconds = Math.ceil((endOfDay.getTime() - now.getTime()) / 1000);
            const payload = {
                data: isNewDay
            ...
            await redisClient.setEx(`quizzer-update-daily-quiz-limit`, ttlInSeconds, JSON.stringify(payload));
